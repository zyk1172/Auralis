...
iOS 最低版本提升至 26.0（iPad 与 iPhone 共用原生 `NavigationStack` Shell）。App 版本 `1.1.0 (4)`，
以 `project.yml` 的 `MARKETING_VERSION` / `CURRENT_PROJECT_VERSION` 为唯一事实来源。

### Added

- **Music Haptics（全新模块）**：随音乐输出触觉反馈，含前瞻预分析（lookahead）、实时 tap 采集、
  感知混音器（v2 / v2.1 / v2.2 平滑）与事件调度；支持本地文件与远程流两类分析源。
  - 身份匹配优先采用 Apple Music / ISRC 等权威标识，其次才回退到标题-艺人文本匹配。
  - 分析结果按曲目持久化缓存，重复播放不重复分析。
- **macOS 端界面重做**：新增独立的资料库（歌曲 / 专辑 / 艺人 / 流派 / 歌单）与详情页视图层，
  补齐迷你播放器与主页布局编辑器；移除旧的单体 `MacAuralisRootView` / `MacInspector` / `MacPages` 结构。
- **AI 助手能力**：支持推理模型（reasoning）与 Responses / Chat 双协议路径，
  接入 Tavily 联网证据与引用映射，支持工具热重载与模型能力探测。

### Changed

- **Agent 运行时**：工具循环（ToolLoop）改用显式意图 / 风险 / 作用域与预算模型，
  多轮工具调用的状态收敛改为诊断化——计数不再硬性中断正常任务，仅作为诊断信号。
  - 强化隐私边界与状态化技能的工具准入反馈，避免越权与静默失败。
- **触觉与音频隔离**：Music Haptics 不再挂进 `AVPlayer` 音频图，
  分析任务受播放状态约束并加入背压与预算上限，避免抢占播放与解码资源。
- **UI 触觉降级**：界面反馈触觉改为「用户操作后的尽力而为（best-effort）」，不阻塞交互主流程。
- **iOS Dock**：浮动 Dock 改为紧凑交互树，滚动内容直接获得安全区间距，
  修复进度条与安全区不同步的问题。
- 全局文案与本地化刷新：统一产品名称与隐私、艺人等标签文案，补齐缺失的本地化字符串。

### Fixed

- Music Haptics 在后台音频播放期间被错误挂起。
- 远程触觉分析的输入格式处理与解码失败恢复。
- iOS 启动时序与触觉输出交接，以及启动图标显示。

### 已知限制（沿用既有说明）

- 连续播放为「尽力无缝」衔接，不宣称样本级 gapless。
- 触觉分析依赖设备能力与环境，为 best-effort 行为；不支持时静默降级，不影响音频播放。
