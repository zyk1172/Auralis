...
| CI-005 | P1 / 高 | GitHub `macos-15` 的 Xcode 16.4 Swift 6.1.2 在默认 `-j8` 编译大型 AppShell/AgentKit 模块时出现前端 `signal 11`，并非源码错误。 | `.github/workflows/ci.yml`：SwiftPM 单测与 AV 目标使用 `--jobs 1` 串行编译，保留完整测试/目标覆盖，避免 Runner 前端并发崩溃。 | 本地 SwiftPM 默认编译、AV 目标编译与全套回归通过；最新 CI 运行需重新确认。 |
