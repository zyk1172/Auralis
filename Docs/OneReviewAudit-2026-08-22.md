...
| CI-005 | P1 / 高 | GitHub `macos-15` 的 Xcode 16.4 Swift 6.1.2 在编译大型 AppShell/AgentKit 模块时出现前端 `signal 11`，并非源码诊断错误；降低 `-j` 仍会复现。 | `.github/workflows/ci.yml`：两个 macOS job 固定到 GitHub `xcode-27` arm64 runner，与本机 Xcode 27 验证工具链一致；SwiftPM 单测与 AV 目标继续使用 `--jobs 1` 串行编译。 | 本地 SwiftPM 默认编译、AV 目标编译与全套回归通过；推送后以 Xcode 27 CI 运行重新确认。 |
