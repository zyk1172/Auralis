...
【Xcode Beta 操作步骤】设置 → Agent → 配置大模型 → 高级设置：把“上下文窗口”与“单次输出上限”改为模型实际值（可用 32K/64K/更大档位）；保存后跑一次歌曲鉴赏与一次推荐索引 V2 批次。再导出/导入一次设置备份，确认这两个值随备份恢复；用旧备份（无这两个字段）导入，确认回退到兼容默认值，且新配置不会被 Agent 层再次缩小。
...
【Xcode Beta 操作步骤】打开设置（Command-,）→ AI 助手：确认“高级设置”分区只有一份，包含“上下文窗口”与“单次输出上限”两个 Stepper，没有重复分区。修改上下文窗口为 32768、输出上限为 4096；执行一次歌曲鉴赏或推荐索引 V2 批次，确认请求按新配置进行（如配置 DeepSeek 128K 模型时不再按 256K 假设）。再导出/导入一次设置备份，确认这两个值随备份恢复；把档位改为更大值重复一次，确认兼容回退也不会把输出偷偷截回 16K。
