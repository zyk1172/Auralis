    ...
    var body: some View {
        SettingsDetailForm(title: "Music Haptics", theme: theme) {
            ...
            Section("Runtime") {
                ...
                LabeledContent("remoteAnalysisPosition", value: String(format: "%.2f", displayDiagnostics.remoteAnalysisPosition))
                LabeledContent("realtimeAnalysisPosition", value: String(format: "%.2f", displayDiagnostics.realtimeAnalysisPosition))
                LabeledContent("analysisLeadSeconds", value: String(format: "%.2f", displayDiagnostics.analysisLeadSeconds))
                LabeledContent("remoteAnalysisSpeedX", value: String(format: "%.2fx", displayDiagnostics.remoteAnalysisSpeedX))
                LabeledContent("realtimeAnalysisSpeedX", value: String(format: "%.2fx", displayDiagnostics.realtimeAnalysisSpeedX))
                ...
                LabeledContent("currentEventSource", value: displayDiagnostics.currentEventSource.rawValue)
                LabeledContent("remoteDecoderState", value: displayDiagnostics.remoteDecoderState.rawValue)
                LabeledContent("decoderFailureStage", value: displayDiagnostics.decoderFailureStage ?? "none")
                LabeledContent("decoderFailureDomain", value: displayDiagnostics.decoderFailureDomain ?? "none")
                LabeledContent("decoderFailureCode", value: displayDiagnostics.decoderFailureCode.map(String.init) ?? "none")
                LabeledContent("droppedAudioDuration", value: String(format: "%.3fs", displayDiagnostics.droppedAudioDuration))
                ...
                LabeledContent("beatGridConfidence", value: String(format: "%.2f", displayDiagnostics.beatGridConfidence))
                LabeledContent("beatGridBPM", value: displayDiagnostics.beatGridBPM.map { String(format: "%.1f", $0) } ?? "none")
                LabeledContent("beatPhaseError", value: String(format: "%.3f", displayDiagnostics.beatPhaseError))
                ...
                LabeledContent("dominantTransientCount", value: "\(displayDiagnostics.dominantTransientCount)")
                LabeledContent("suppressedTransientCount", value: "\(displayDiagnostics.suppressedTransientCount)")
                LabeledContent("suppressedHighPercussionCount", value: "\(displayDiagnostics.suppressedHighPercussionCount)")
                LabeledContent("mergedCollisionCount", value: "\(displayDiagnostics.mergedCollisionCount)")
                LabeledContent("activeTextureType", value: displayDiagnostics.activeTextureType?.rawValue ?? "none")
                LabeledContent("continuousDutyCycle", value: String(format: "%.3f", displayDiagnostics.continuousDutyCycle))
                LabeledContent("perceptualEventsPerSecond", value: String(format: "%.3f", displayDiagnostics.perceptualEventsPerSecond))
                LabeledContent("fatigueGain", value: String(format: "%.3f", displayDiagnostics.fatigueGain))
                LabeledContent("hapticCommitHorizon", value: String(format: "%.2fs", displayDiagnostics.hapticCommitHorizon))
                LabeledContent("hapticDriftSeconds", value: String(format: "%.3fs", displayDiagnostics.hapticDriftSeconds))
                LabeledContent("driftGuardBand", value: displayDiagnostics.driftGuardBand.rawValue)
