...
///
/// iOS 呈现策略：用 NavigationLink 推入整页（导出页 / 恢复页），
/// 彻底规避「Form 行内直接弹 sheet / fileImporter 首次点击被撤销、要点两下才有效」的
/// SwiftUI 问题——与 AI 配置「iOS 上从 Form 直接弹 sheet 存在点击无响应，改为推入整页」同一处理。
