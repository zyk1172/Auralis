...
public struct RecommendationIndexPackage: Codable, Sendable, Equatable {
    ...
    public var entries: [RecommendationIndexPackageEntry]
    ...
    public init(
        ...
        entries: [RecommendationIndexPackageEntry]
    ) {
...
public struct RecommendationIndexPackageEntry: Codable, Sendable, Equatable {
    public var remoteTrackID: String
    public var contentHash: String
    public var tags: [RecommendationIndexPackageTag]
    ...
    public init(
        ...
        contentHash: String,
        tags: [RecommendationIndexPackageTag],
        classifier: String,
...
public struct RecommendationIndexPackageTag: Codable, Sendable, Equatable {
...
public struct RecommendationIndexImportStatistics: Sendable, Equatable {
...
public enum RecommendationIndexImportError: Error, Equatable, Sendable {
    case invalidData
    case tooLarge(Int)
...
extension RecommendationIndexImportError: LocalizedError {
...
extension LocalCatalogStore {
    ...
    /// 传输容器结构版本。
    public static let recommendationIndexPackageFormatVersion = 1

    /// 只导出当前服务器、当前规则版本 + 当前内容指纹版本 + 已有完整 tags 的歌曲。
    /// pending / invalid / 损坏记录一律不导出。
    public func exportRecommendationIndexPackage(serverID: ServerID) throws -> RecommendationIndexPackage {
        ...
        guard !valid.isEmpty else {
            return RecommendationIndexPackage(
                formatVersion: Self.recommendationIndexPackageFormatVersion,
        ...
        var tagsByID: [String: [RecommendationIndexPackageTag]] = [:]
        for row in tagRows {
        ...
        let entries = valid.compactMap { line -> RecommendationIndexPackageEntry? in
            guard let remoteID = GlobalID(line.id)?.remoteID else { return nil }
            let tags = tagsByID[line.id] ?? []
            guard !tags.isEmpty else { return nil }
            let state = stateByID[line.id]
            return RecommendationIndexPackageEntry(
                remoteTrackID: remoteID,
                contentHash: recommendationIndexContentHash(line),
                tags: tags,
    ...
    public func importRecommendationIndexPackage(
        data: Data,
        serverID: ServerID
    ) throws -> RecommendationIndexImportStatistics {
        let maxBytes = 50 * 1024 * 1024
        guard data.count <= maxBytes else { throw RecommendationIndexImportError.tooLarge(data.count) }

        let package: RecommendationIndexPackage
        do {
            package = try JSONDecoder().decode(RecommendationIndexPackage.self, from: data)
        } catch {
            throw RecommendationIndexImportError.invalidData
        }
        guard package.formatVersion == Self.recommendationIndexPackageFormatVersion,
              package.rulesVersion == RecommendationIndex.rulesVersion,
              package.contentHashVersion == RecommendationIndex.contentHashVersion
        else {
            throw RecommendationIndexImportError.versionIncompatible(
        ...
        guard package.entries.count <= maxEntries else {
            throw RecommendationIndexImportError.tooManyEntries(package.entries.count)
        }
        ...
        var statistics = RecommendationIndexImportStatistics(totalEntries: package.entries.count)

        // 先逐条校验并分类，收集需要写入的记录；整批在一个事务内落库。
        var toImport: [(line: CatalogTrackLine, entry: RecommendationIndexPackageEntry)] = []
        for entry in package.entries {
            ...
            let localHash = recommendationIndexContentHash(line)
            ...
            if existingVersion >= RecommendationIndex.contentHashVersion,
    ...
    private func validatedTags(_ tags: [RecommendationIndexPackageTag]) -> [(dimension: String, value: String)] {
        var seen = Set<String>()
